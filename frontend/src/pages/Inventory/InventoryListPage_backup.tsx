...
const InventoryListPage: React.FC = () => {
  ...
  return (
    <div style={{ padding: '16px' }}>
        ...
        footer={[
          user?.role === 'admin' && selectedInventoryDetail && (
            <Button
              ...
              onClick={() => {
                setEditingInventory({
                  product_id: selectedInventoryDetail.product_id,
                  product_name: selectedInventoryDetail.product_name,
                  brand: selectedInventoryDetail.brand,
                  category: selectedInventoryDetail.category,
                  sku_code: selectedInventoryDetail.sku_code,
                  sizes: selectedInventoryDetail.size_inventories?.map((item: any) => ({
                    size: item.size,
                    quantity: item.quantity,
                    inventory_id: item.id
                  })) || []
                  ...
                  <div style={{ padding: 16, backgroundColor: '#f0f2f5', borderRadius: 8 }}>
                    <Row gutter={16}>
                      ...
                      <Col span={16}>
                        ...
                        <div>
                          <strong style={{ color: '#595959' }}>창고 위치:</strong>{' '}
                          <span style={{ fontSize: 15 }}>
                            {(() => {
                              const locations = selectedInventoryDetail.size_inventories?.map((item: any) => item.location || '-') || [];
                              const uniqueLocations = Array.from(new Set(locations.filter((loc: string) => loc !== '-')));
                ...
                <div>
                  ...
                  {(() => {
                    ...
                    selectedInventoryDetail.size_inventories?.forEach((item: any) => {
                      sizeMap.set(item.size, { quantity: item.quantity, location: item.location });
                    });
      ...
      <Modal
        title={<span style={{ fontSize: 18, fontWeight: 700 }}>재고 수정</span>}
        ...
        {editingInventory && (
          <Form
            ...
            onFinish={async (values) => {
              try {
                for (const sizeInfo of editingInventory.sizes) {
                  const newQuantity = values[`size_${sizeInfo.size}`];
                  const currentQuantity = sizeInfo.quantity;
                  const quantityChange = newQuantity - currentQuantity;

                  if (quantityChange !== 0) {
                    await inventoryService.adjustInventoryQuantity(
                      sizeInfo.inventory_id,
                      quantityChange,
                      '관리자 재고 수정'
                    );
                  }
          ...
          >
            <Row gutter={24} style={{ marginBottom: 24 }}>
                ...
                <div>
                  ...
                  <div style={{ padding: 16, backgroundColor: '#f0f2f5', borderRadius: 8 }}>
                    <Row gutter={16}>
                      <Col span={8}>
                        {(() => {
                          const imagePath = editingInventory.brand && editingInventory.sku_code
                            ? getFileUrl(`/uploads/products/${editingInventory.brand}/${editingInventory.sku_code}.png`)
                            : null;
                          return imagePath ? (
                            <img
                              src={imagePath}
                              alt={editingInventory.product_name}
                              style={{
                                width: '100%',
                                height: 'auto',
                                objectFit: 'cover',
                      ...
                      <Col span={16}>
                        <div style={{ marginBottom: 12 }}>
                          <strong style={{ color: '#595959' }}>상품명:</strong>{' '}
                          <span style={{ fontSize: 15, fontWeight: 500 }}>{editingInventory.product_name}</span>
                        </div>
                        <div style={{ marginBottom: 12 }}>
                          <strong style={{ color: '#595959' }}>브랜드:</strong>{' '}
                          <span style={{ fontSize: 15 }}>{editingInventory.brand || '-'}</span>
                        </div>
                        <div style={{ marginBottom: 12 }}>
                          <strong style={{ color: '#595959' }}>카테고리:</strong>{' '}
                          <span style={{ fontSize: 15 }}>{editingInventory.category || '-'}</span>
                        </div>
                        <div>
                          <strong style={{ color: '#595959' }}>상품 번호:</strong>{' '}
                          <span style={{ fontSize: 15 }}>{editingInventory.sku_code || '-'}</span>
                        </div>
                      </Col>
                    </Row>
                  </div>
              ...
              <Col span={12}>
                <div>
                  ...
                  {(() => {
                    const fixedSizes = editingInventory.category === 'shoes'
                      ? ['220', '225', '230', '235', '240', '245', '250', '255', '260', '265', '270', '275', '280', '285', '290', '295', '300', '305', '310', '315']
                      : ['XXS', 'XS', 'S', 'M', 'L', 'XL', 'XXL', 'XXXL'];

                    const sizeMap = new Map();
                    editingInventory.sizes.forEach((sizeInfo: any) => {
                      sizeMap.set(sizeInfo.size, {
                        quantity: sizeInfo.quantity,
                        inventory_id: sizeInfo.inventory_id
                      });
                    });

                    const firstRow = editingInventory.category === 'shoes' ? fixedSizes.slice(0, 10) : fixedSizes;
                    const secondRow = editingInventory.category === 'shoes' ? fixedSizes.slice(10) : [];

                    const renderSizeRow = (sizes: string[]) => (
                    ...
                    return (
                      <div>
                        {renderSizeRow(firstRow)}
                        {secondRow.length > 0 && renderSizeRow(secondRow)}
                      </div>
                    );
                  })()}
                </div>
            ...
            <Form.Item style={{ marginTop: 24, marginBottom: 0, textAlign: 'right' }}>
              <Space>
                <Button onClick={() => { setEditModalVisible(false); editForm.resetFields(); }}>
                  취소
                </Button>
                <Button type="primary" htmlType="submit">
                  저장
